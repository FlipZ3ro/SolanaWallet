...
final class CryptoHelper {
    ...
    func generateMnemonic(wordCount: Int = 24) -> String {
        // BIP39 mnemonic generation
        // In production, use a proper BIP39 library
        let entropy = generateEntropy(bits: wordCount == 24 ? 256 : 128)
        return entropyToMnemonic(entropy)
    }

    private func generateEntropy(bits: Int) -> Data {
    ...
    private func entropyToMnemonic(_ entropy: Data) -> String {
        // Simplified - in production use proper BIP39 word list
        ...
        let words = [
            ...
            "prize", "process", "produce", "profit", "program", "project", "promote", "proof",
        ...
        // Use entropy to select words (simplified)
        var selectedWords: [String] = []
        let bytes = [UInt8](entropy)

        for i in 0..<(entropy.count * 8 / 11) {
            let byteIndex = i * 11 / 8
            let bitOffset = i * 11 % 8

            guard byteIndex < bytes.count else { break }

            var value = Int(bytes[byteIndex]) << 8
            if byteIndex + 1 < bytes.count {
                value |= Int(bytes[byteIndex + 1])
            }
            value = (value >> (8 - bitOffset)) & 0x7FF

            let wordIndex = value % words.count
    ...
    func HMAC_SHA512(key: Data, data: Data) -> Data {
